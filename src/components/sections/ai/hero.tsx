  ...
  return (
    <section className="bg-[radial-gradient(110%_80%_at_80%_0%,rgba(115,73,245,.18),rgba(17,14,9,0)_58%),radial-gradient(80%_70%_at_10%_100%,rgba(242,92,5,.16),rgba(17,14,9,0)_65%)] pt-[clamp(120px,16vh,180px)] pb-[clamp(56px,8vw,96px)]">
      <div className="mx-auto grid w-[min(1360px,100%-2*clamp(20px,4vw,64px))] items-end gap-10 lg:grid-cols-[1.05fr_.95fr] lg:gap-16">
        ...
        <Reveal visibleOnLoad delay={180} className="relative hidden aspect-[4/3] overflow-hidden rounded-[32px] border border-mi-cream/14 lg:block">
          <img src={travelerImage} alt="Traveller using technology while on the move" className="size-full object-cover" />
          <div className="absolute inset-0 bg-gradient-to-tr from-background-dark/55 via-transparent to-[#7349F5]/25" />
          <div className="absolute bottom-5 left-5 rounded-full border border-mi-cream/20 bg-background-dark/70 px-4 py-2 font-mi-body text-[.68rem] font-bold tracking-[.1em] text-mi-cream/80 backdrop-blur-sm">INTELLIGENCE IN MOTION</div>
        </Reveal>
      </div>
    </section>
  );
